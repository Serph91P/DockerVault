...
async def get_encryption_status(db: AsyncSession = Depends(get_db)):
    ...
    return EncryptionStatusResponse(
        ...
        key_created_at=config.key_created_at.isoformat() if config.key_created_at else None,
    )
...
async def setup_encryption(db: AsyncSession = Depends(get_db)):
    ...
    if existing and existing.setup_completed:
        raise HTTPException(
            status_code=400,
            detail="Encryption already configured. Use /regenerate to create new keys (this will make existing backups unrecoverable!)."
        )
...
async def confirm_setup(
    ...
    if not config:
        raise HTTPException(
            status_code=400,
            detail="Encryption not set up. Call /setup first."
        )
    
    config.setup_completed = True
    await db.commit()
    
    return {"message": "Encryption setup completed. Your backups will now be encrypted."}
...
async def regenerate_keys(
    ...
    if not confirm_data_loss:
        raise HTTPException(
            status_code=400,
            detail="You must confirm that you understand existing backups will become unrecoverable."
        )
    ...
    try:
        ...
        logger.warning("Encryption keys regenerated - old backups may be unrecoverable!")
