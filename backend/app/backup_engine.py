    ...
    async def validate_backup_prerequisites(
        ...
        if target.target_type == "container" and target.container_name:
            ...
            if not any(c.name == target.container_name for c in containers):
                ...
                logger.warning(
                    f"Validation failed: container not found",
                    extra={
                        "target_id": target.id,
            ...
            if not any(v.name == target.volume_name for v in volumes):
                ...
                logger.warning(
                    f"Validation failed: volume not found",
                    extra={
                        "target_id": target.id,
                        "target_name": target.name,
            ...
            if not os.path.exists(target.host_path):
                ...
                logger.warning(
                    f"Validation failed: path not found",
                    extra={
                        "target_id": target.id,
                        "target_name": target.name,
                ...
                logger.warning(
                    f"Validation failed: path not readable",
                    extra={
                        "target_id": target.id,
                        "target_name": target.name,
                        "host_path": target.host_path,
        ...
        if target.dependencies:
            ...
            for dep in target.dependencies:
                if dep not in container_names:
                    ...
                    logger.warning(
                        f"Validation failed: dependency not found",
                        extra={
                            "target_id": target.id,
                            "target_name": target.name,
                            "dependency": dep,
