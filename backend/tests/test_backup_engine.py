...
class TestBackupMetrics:
    ...
    def test_metrics_memory_limit(self):
        ...
        for i in range(150):
            metrics.record_backup(target_id=1, duration=float(i), size=i * 100, success=True)
...
class TestBackupValidation:
    ...
    @patch('app.backup_engine.docker_client')
    async def test_validate_low_disk_space(self, mock_docker, mock_disk_usage, db_session, temp_backup_dir):
    ...
    async def test_validate_existing_container(self, mock_docker, db_session):
        ...
        assert not any("container" in issue.lower() and "not found" in issue.lower() for issue in issues)
    ...
    async def test_concurrent_backup_limit(self, db_session):
        ...
        for i in range(5):
            ...
            backup = Backup(
                ...
                backup_type=BackupType.FULL,
                status=BackupStatus.PENDING,
                backup_metadata={"target_name": f"test-volume-{i}", "target_type": "volume"}
            )
            db_session.add(backup)
            await db_session.commit()
            await db_session.refresh(backup)
...
class TestBackupContainerDependencies:
    ...
    async def test_backup_stops_containers_in_order(self, mock_docker, db_session):
        ...
        mock_docker.list_volumes = AsyncMock(return_value=[
            MagicMock(name="test-volume", mountpoint="/var/lib/docker/volumes/test-volume/_data")
        ])
        ...
        with patch('app.backup_engine.settings') as mock_settings:
            ...
            with patch.object(engine, '_create_backup_archive', AsyncMock(return_value="/tmp/backup.tar.gz")):
                with patch.object(engine, '_calculate_checksum', AsyncMock(return_value="abc123")):
